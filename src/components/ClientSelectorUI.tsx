...
import type { Client, ClientListDisplayProps, LetterIndexProps, SearchInputProps } from "~/types/types"; 
 ...
 */
export const DrawerTrigger: Component<{ id: string, buttonText: string }> = (props) => {
    return (
 ...
 */
export const DrawerHeader: Component<{title: string; onClose: () => void;}> = (props) => {
    return (
 ...
 * @component
 * @param {Object} props - The properties passed to the component.
 * @param {Client} props.client - The client object to display.
 * @param {(client: Client) => void} props.onSelect - Callback function triggered when the client is selected.
 * @returns {JSX.Element} A button element representing a client.
 */
export const ClientListItem: Component<{ client: Client; onSelect: (client: Client) => void; }> = (props) => {
    return (
        <button
            onClick={() => props.onSelect(props.client)}
...
// --- Pure: Client List Display ---
/**
 * Renders a list of clients or a fallback message if no clients are found.
 * 
 * @component
 * @param {ClientListDisplayProps} props - The properties passed to the component.
 * @returns {JSX.Element} A list of clients or a fallback message.
 */
export const ClientListDisplay: Component<ClientListDisplayProps> = (props) => {
    return (
        <Show
        ...
        >
            <For each={props.clients}>
                {(client) => <ClientListItem client={client} onSelect={props.onClientSelect} />}
            </For>
        </Show>
    );
};
